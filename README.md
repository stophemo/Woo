...
| 平台 | 下载 | 说明 |
|------|------|------|
| 🍎 macOS | [DMG (Apple Silicon)](https://github.com/stophemo/Woo/releases/latest) | 需安装自签证书（见 Release 安装说明） |
| 🪟 Windows | [NSIS 安装包 (x64)](https://github.com/stophemo/Woo/releases/latest) | 双击安装 |
| 🤖 Android | [APK (ARM64)](https://github.com/stophemo/Woo/releases/latest) | 允许安装未知来源应用 |
| 🐧 Linux | 即将推出 | 或从源码编译（见下方） |

> 💡 **macOS 用户**：Woo 使用自签证书签名。首次使用请下载 Release 附带的 `woo-codesign.cer`，双击导入钥匙串 → 选择"系统"钥匙串 → 输入密码 → 在"钥匙串访问"中找到"Woo Notes Root CA" → 右键"显示简介" → "信任" → "使用此证书时"设为"始终信任"。之后所有 Woo 版本自动受信。
