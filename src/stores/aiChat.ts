...
export const useAiChatStore = defineStore('aiChat', () => {
  ...
  function getAgent(): Agent {
    ...
    agent.setContextProvider({
      ...
      searchDocuments: async (query: string) => {
        try {
          const docs = await invoke<any[]>('document:search', { keyword: query })
      ...
      getDocumentDetail: async (title: string) => {
        try {
          // 先通过搜索找到匹配标题的文档
          const docs = await invoke<any[]>('document:search', { keyword: title })
          const match = docs?.find((d: any) => d.title === title)
          if (!match) {
            // 模糊匹配第一个
            const fuzzy = docs?.[0]
            if (!fuzzy) return null
            const detail = await invoke<any>('document:get', { id: fuzzy.id })
          ...
          const detail = await invoke<any>('document:get', { id: match.id })
          if (!detail) return null
          const created = detail.create_time ? new Date(detail.create_time).toLocaleString('zh-CN') : '未知'
          const updated = detail.update_time ? new Date(detail.update_time).toLocaleString('zh-CN') : '未知'
          const contentLen = detail.content ? detail.content.replace(/<[^>]+>/g, '').length : 0
      ...
      findFolderByName: async (name: string) => {
        try {
          const tree = await invoke<any[]>('folder:tree')
          const queue = [...tree]
      ...
      findNoteByTitle: async (title: string) => {
        try {
          const docs = await invoke<any[]>('document:search', { keyword: title })
          // 精确匹配优先
          const exact = docs?.find((d: any) => d.title === title)
      ...
      createNote: async (title: string, folderId?: string) => {
        try {
          ...
          const doc = await invoke<any>('document:create', { title, folderId: targetFolderId })
          if (doc?.id) {
      ...
      updateNote: async (noteId: string, content: string) => {
        ...
        try {
          ...
          // 最终写入后端
          await invoke('document:updateContent', { id: noteId, content: html })
          return true
        } catch {
          return false
        } finally {
      ...
      deleteNote: async (noteId: string) => {
        try {
          await invoke('document:remove', { id: noteId })
      ...
      createFolder: async (name: string, parentId?: string) => {
        try {
          const id = await invoke<string>('folder:create', { name, parentId: parentId || null })
      ...
      renameFolder: async (folderId: string, newName: string) => {
        try {
          await invoke('folder:rename', { folderId, name: newName })
          // 立即更新响应式状态
          const ws = useWorkspaceStore()
      ...
      deleteFolder: async (folderId: string) => {
        try {
          await invoke('folder:remove', { folderId })
      ...
      listRecentNotes: async (limit?: number) => {
        try {
          const docs = await invoke<any[]>('document:listAll')
  ...
  async function rebuildKb(): Promise<{ totalDocs: number; totalChunks: number; embedSuccess?: number }> {
    ...
    try {
      const result = await invoke<{ totalDocs: number; totalChunks: number; embedSuccess: number }>('kb:rebuild')
  ...
  async function refreshKbStatus() {
    try {
      const st = await invoke<{ docCount: number; chunkCount: number; embedCount: number }>('kb:status')
  ...
  async function searchKb(query: string, limit?: number): Promise<string> {
    try {
      const chunks = await invoke<any[]>('kb:search', { query, limit: limit || 6 })
